...
export function createBorder(
  ...
  const lines = rawLines.flatMap((line) => wrapLine(line, targetWidth));
  ...
  const topBorder =
    border.topLeft + centerPattern(border.top, contentWidthPx, measureWidth) + border.topRight;
  const bottomBorder =
    border.bottomLeft +
    centerPattern(border.bottom, contentWidthPx, measureWidth) +
    border.bottomRight;
